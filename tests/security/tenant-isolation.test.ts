...
describe("tenant isolation", () => {
  ...
  describe("reads cannot cross the boundary", () => {
    test("search never returns another workspace's records", async () => {
      ...
      const hits = await searchEverything({ workspaceIds: [A.workspaceId], userId: A.ownerId, restrictedWorkspaceIds: [] }, "Bravo");
    ...
    test("contact list is scoped", async () => {
      ...
      const { contacts } = await listContacts({ workspaceIds: [A.workspaceId], userId: A.ownerId, restrictedWorkspaceIds: [] }, {});
      assert.ok(
    ...
    test("dashboard aggregates only in-scope data", async () => {
      ...
      const dashboard = await getDashboard({ workspaceIds: [A.workspaceId], userId: A.ownerId, restrictedWorkspaceIds: [] }, null);
    ...
    test("analytics aggregates only in-scope data", async () => {
      ...
      const a = await getAnalytics({ workspaceIds: [A.workspaceId], userId: A.ownerId, restrictedWorkspaceIds: [] }, 365);
      const both = await getAnalytics({ workspaceIds: [A.workspaceId, B.workspaceId], userId: A.ownerId, restrictedWorkspaceIds: [] }, 365);
    ...
    test("record fetch for another workspace's record returns nothing", async () => {
      ...
      const contact = await getContact({ workspaceIds: [A.workspaceId], userId: A.ownerId, restrictedWorkspaceIds: [] }, B.contactId);
