...
async function main() {
  ...
  const lower = await searchEverything({ workspaceIds: [workspaceId], userId, restrictedWorkspaceIds: [] }, "alpha");
  const upper = await searchEverything({ workspaceIds: [workspaceId], userId, restrictedWorkspaceIds: [] }, "ALPHA");
  ...
  const listed = await listContacts({ workspaceIds: [workspaceId], userId, restrictedWorkspaceIds: [] }, { q: "beta" });
